...
```shell
curl -i -N -H "Connection: Upgrade" -H "Upgrade: websocket" -H "Sec-WebSocket-Key: dGhlIHNhbXBsZSBub25jZQ==" -H "Sec-WebSocket-Version: 13" http://localhost:8080/ws
```

TEST (work in progress)

Connection to the websocket, initialize the message
- to understand, what is the role of the key, how to the server knows who is accepted or not and why the key has to be generated by the client?
- 
